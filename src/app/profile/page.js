...
export default function Profile() {
  ...
  // Sync form with latest userData whenever it changes (covers realtime updates)
  useEffect(() => {
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      // AuthContext has onSnapshot so it will auto-refresh userData
      setEditing(false);
  ...
  const cancelEdit = () => {
    setEditing(false);
    setForm(userData); // reset to latest saved data
    setPhotoPreview(null);
  };
    ...
    <div className="page-wrap animate-fade">
      ...
      <div className="card" style={{ display:'flex', alignItems:'center', gap:20, marginBottom:16 }}>
        ...
        <div style={{ flex:1, minWidth:0 }}>
          ...
          {userData.nameBengali && (
            <div style={{ fontSize:14, color:'#64748b' }}>{userData.nameBengali}</div>
          )}
      ...
      {editing ? (
        <div className="card">
          <div style={{ display:'grid', gridTemplateColumns:'1fr 1fr', gap:'0 16px' }}>
            <Field label="Full Name (English)" k="nameEnglish" placeholder="John Doe" />
            <Field label="Full Name (Bengali)"  k="nameBengali" placeholder="জন ডো" />
            <Field label="Father's Name"        k="fatherName"  placeholder="Father's name" />
            <Field label="Mother's Name"        k="motherName"  placeholder="Mother's name" />
            ...
            <Field label="NID Number"  k="nid"        placeholder="NID" />
            <Field label="Phone"       k="phone"      type="tel" placeholder="+8801XXXXXXXXX" />
          </div>
          <div className="form-group">
            <label className="form-label">Address</label>
            <textarea rows={2} value={form.address || ''} onChange={e => set('address', e.target.value)}
              placeholder="Full address" style={{ resize:'vertical' }} />
          </div>
        </div>
      ) : (
        <div className="card">
