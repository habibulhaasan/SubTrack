...
// ─── Helpers defined OUTSIDE any component so React never recreates them ───
    ...
    <Modal title="Member Profile" onClose={onClose}>
      ...
      <div style={{ marginBottom: 16 }}>
        ...
        <div style={{ display: 'flex', gap: 8 }}>
          ...
          />
          <button
            onClick={() => onSaveId(member.id, idEdit)}
            disabled={saving === member.id + '_id'}
      ...
      <button
        ...
        style={{
          width: '100%', marginTop: 18, padding: '12px', borderRadius: 8,
...
export default function AdminMembers() {
  ...
  useEffect(() => {
    ...
    const unsub = onSnapshot(collection(db, 'organizations', orgId, 'members'), async snap => {
      ...
      setMembers(merged);
      // Keep selected in sync with live data without closing the modal
        ...
        <div>
          ...
          <div className="page-subtitle">
            ...
            {settings.autoMemberId && (
              <span style={{ marginLeft: 8, fontSize: 11, background: '#eff6ff', color: '#2563eb', padding: '2px 8px', borderRadius: 4, fontWeight: 600 }}>Auto-ID</span>
            )}
          </div>
        </div>
      </div>
